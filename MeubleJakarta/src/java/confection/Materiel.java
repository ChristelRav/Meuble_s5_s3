...
public class Materiel {
    ...
    public Materiel insert(Connection c) throws Exception {
        ...
        try (Statement stmt = c.createStatement()) {
            stmt.executeUpdate("INSERT INTO materiel (materiel,unite,pu) VALUES ('" + this.getMateriel()+"','"+this.getUnite() + "' ,"+this.getQp()+")", Statement.RETURN_GENERATED_KEYS);
            try (ResultSet rs = stmt.getGeneratedKeys()) {
                if (rs.next()) pan = new Materiel(rs.getInt(1), rs.getString(2),rs.getString(3),rs.getDouble(4));
            }
        } catch (Exception e) {
            e.printStackTrace();
        } finally {c.close();}
